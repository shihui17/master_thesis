...
def heuristic_kalman(N, Nbest, n, sample_num, joint):
                ...
                for z in range(6):
                    if z == 1:
                        if u[z] == 1: # positive trapeze, q_opt > q_ref to be encouraged, coeff needs to be <0 to reduce cost function value
                            coeff[z] = -780 if sign_array[z] > 0 else 780
                        ...
                        if u[z] == 1: # positive trapeze, q_opt > q_ref to be encouraged, coeff needs to be <0 to reduce cost function value
                            coeff[z] = -190 if sign_array[z] > 0 else 190
                        else: # negative trapeze, q_opt < q_ref to be encouraged
                            coeff[z] = 190 if sign_array[z] < 0 else -190                         
                    elif z == 5:
                        if u[z] == 1: # positive trapeze, q_opt > q_ref to be encouraged, coeff needs to be <0 to reduce cost function value
                            coeff[z] = -450 if sign_array[z] > 0 else 450
                        else: # negative trapeze, q_opt < q_ref to be encouraged
                            coeff[z] = 450 if sign_array[z] < 0 else -450
                    else: 
                        if u[z] == 1: # positive trapeze, q_opt > q_ref to be encouraged, coeff needs to be <0 to reduce cost function value
    ...
    np.savetxt("discrete_int_q.txt", discrete_q)
    np.savetxt("discrete_int_qd.txt", discrete_qd)
    np.savetxt("discrete_int_qdd.txt", discrete_qdd)
