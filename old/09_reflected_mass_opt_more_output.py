...
def calculate_max_force(q_mat, qd_mat, contact_points_table, m_H, K, F_p):
    ...
    print(f'maximum momentum that occurs during trajectory for each point of interest: {max_momentum}')
    max_of_max_momentum = np.amax(max_momentum)
    print(max_of_max_momentum)    #print(lin_vel)
    max_contact_force = np.amax(equi_contact_force, axis=1)
    print(f'maximum force that occurs during trajectory for each point of interest: {max_contact_force}')
    max_of_all = np.amax(max_contact_force)
    delta_f = (max_of_all - F_p)**2

    return max_contact_force, max_of_all, delta_f, momentum
...
def hka_force_opt(N, Nbest, trajectory_data, start_joint_config, end_joint_config):
    ...
    while iter <= 10:
        ...
        contact_force_list_def = [('Force','f8'), ('Number','i2'), ('Force Diff', 'f8')] # define a tuple that contains the energy consumption and the corresponding row index
        ...
        for i in range(N): # iterate through trajectory set
            ...
            max_contact_force = force_data[3]
            #print(max_contact_force[1])
